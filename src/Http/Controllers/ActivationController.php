        ...
        try {
            ...
            if (!$user->attemptActivation($code)) {
                return Redirect::to(Config::get('credentials.home', '/'))
                    ->with('error', trans('credentials.there_was_a_problem_activating_this_account'));
            }

            $user->addGroup(Credentials::getGroupProvider()->findByName('Users'));

            return Redirect::route('account.login')
                ->with('success', trans('credentials.your_account_has_been_activated_successfully'));
        } catch (UserNotFoundException $e) {
            return Redirect::to(Config::get('credentials.home', '/'))
                ->with('error', trans('credentials.there_was_a_problem_activating_this_account'));
        } catch (UserAlreadyActivatedException $e) {
            return Redirect::route('account.login')
                ->with('warning', trans('credentials.you_have_already_activated_this_account'));
        }
    }
    ...
    public function postResend()
    {
        ...
        try {
            ...
            return Redirect::route('account.resend')
                ->with('success', trans('credentials.check_your_email_for_your_new_activation_email'));
        } catch (UserNotFoundException $e) {
            return Redirect::route('account.resend')
